...
public class Playing extends State implements Statemethods {
	...
	public Playing(Game game) {
		...
		if (levelManager.getLevelIndex() == 0) {
			  backgroundImg = LoadSave.GetSpriteAtlas(LoadSave.LVL1_BG);
			} 
	...
	public void loadNextLevel() {
		...
		drawShip = false;
		 if (levelManager.getLevelIndex() == 1) {
			  // Remove previous level's background image
			  backgroundImg = null;
			  backgroundImg = LoadSave.GetSpriteAtlas(LoadSave.LVL2_BG);
			} else if (levelManager.getLevelIndex() == 2) {
			  // Remove previous level's background image
			  backgroundImg = null;
			  backgroundImg = LoadSave.GetSpriteAtlas(LoadSave.LVL3_BG);
			} else if (levelManager.getLevelIndex() == 3) {
			  // Remove previous level's background image
			  backgroundImg = null;
			  backgroundImg = LoadSave.GetSpriteAtlas(LoadSave.LVL4_BG);
			} else if (levelManager.getLevelIndex() == 4) {
			  // Remove previous level's background image
			  backgroundImg = null;
			  backgroundImg = LoadSave.GetSpriteAtlas(LoadSave.LVL5_BG);
			}
	}
	...
	public void draw(Graphics g) {
		...
		//Loads the backgrounds for each level, depending on the level number
		int levelNumber = levelManager.getCurrentLevelNumber();
		if (levelNumber == 1) {
		    g.setColor(Color.WHITE);
		    g.setFont(new Font("Arial", Font.BOLD, 16));
		    g.drawString("Level " + levelNumber + ": Battle of Castle Black Pt. 1", Game.GAME_WIDTH - 280, 20); 
		}
		if (levelNumber == 2) {
		    g.setColor(Color.WHITE);
		    g.setFont(new Font("Arial", Font.BOLD, 16));
		    g.drawString("Level " + levelNumber + ": Battle of Castle Black Pt. 2", Game.GAME_WIDTH - 280, 20);
		}
		if (levelNumber == 3) {
		    g.setColor(Color.WHITE);
		    g.setFont(new Font("Arial", Font.BOLD, 16));
		    g.drawString("Level " + levelNumber + ": Battle of the Bastards Pt. 1", Game.GAME_WIDTH - 280, 20); 
		}
		if (levelNumber == 4) {
		    g.setColor(Color.WHITE);
		    g.setFont(new Font("Arial", Font.BOLD, 16));
		    g.drawString("Level " + levelNumber + ": Battle of the Bastards Pt. 2", Game.GAME_WIDTH - 280, 20);
		}
		if (levelNumber == 5) {
		    g.setColor(Color.WHITE);
		    g.setFont(new Font("Arial", Font.BOLD, 16));
		    g.drawString("Level " + levelNumber + ": Battle of Winterfell", Game.GAME_WIDTH - 220, 20); 
		}
	}
	//method to draw clouds. However, we decided to take them out of the game.
